...
def decode(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
    ...
    for t in range(max_out_len):
        ...
        output = model.decoder(encoder_outputs, src_pad_mask, generated, trg_pad_mask).to(device)
...
def beam_search_decode(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
                       tgt_tokenizer: spm.SentencePieceProcessor, args, device: torch.device, beam_size: int = 5, alpha: float = 0.7, local_threshold: float = 0.5, maximum_candidates: int = 3):
    ...
    for _ in range(max_out_len):
        ...
        for seq, score in beams:
            ...
            with torch.no_grad():
                ...
                logits = model.decoder(encoder_outputs, src_pad_mask, seq, trg_pad_mask)[:, -1, :]
...
def beam_search_decode_rlp(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
                           tgt_tokenizer: spm.SentencePieceProcessor, args, device: torch.device, beam_size: int = 5, alpha: float = 0.7, local_threshold: float = 0.5, maximum_candidates: int = 3):
    """Beam Search decoding compatible with Transformer-based Seq2Seq models.
    uses relative local threshold pruning strategy"""
    model.eval()
    BOS, EOS, PAD = tgt_tokenizer.bos_id(), tgt_tokenizer.eos_id(), tgt_tokenizer.pad_id()
    # __QUESTION 1: what does this line set up and why is the beam represented this way?
    beams = [(torch.tensor([[BOS]], device=device), 0.0)]
...
def beam_search_decode_mcn(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
                           tgt_tokenizer: spm.SentencePieceProcessor, args, device: torch.device, beam_size: int = 5, alpha: float = 0.7, local_threshold: float = 0.5, maximum_candidates: int = 3):
    """Beam Search decoding compatible with Transformer-based Seq2Seq models.
    uses maximum candidates per node pruning strategy with a default threshold of 3"""
    model.eval()
    BOS, EOS, PAD = tgt_tokenizer.bos_id(), tgt_tokenizer.eos_id(), tgt_tokenizer.pad_id()
    # __QUESTION 1: what does this line set up and why is the beam represented this way?
    beams = [(torch.tensor([[BOS]], device=device), 0.0)]
    ...
    for _ in range(max_out_len):
        new_beams = []
        for seq, score in beams:  # i.e. per history
            if seq[0, -1].item() == EOS:
                new_beams.append((seq, score))
                continue
            with torch.no_grad():
            ...
            for k in range(maximum_candidates):
                # __QUESTION 4: explain the tensor shapes and the logic when creating new_seq and new_score below. Is any broadcasting or indexing issue possible?
                new_seq = torch.cat([seq, topk_ids[:, k].unsqueeze(0)], dim=1)
                new_score = score + topk_log_probs[:, k].item()  # stores the probability
                new_beams.append((new_seq, new_score))
